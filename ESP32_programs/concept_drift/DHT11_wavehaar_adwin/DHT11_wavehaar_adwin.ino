...
void loop() {
  ...
  if(serieS.getSize() < 32){
    ...
    haarT.transform = serieT;
    haarU.transform = serieU;
    haarS.transform = serieS;
    ...
    mudaU = adwinU.update(h);
    mudaS = adwinS.update(hic);
    ...
    //verifica se O comportamento da umidade mudou
    else if(mudaU == 1){
      Serial.println(F("O comportamento da umidade mudou"));
    }

    //verifica se O comportamento da sensação térmica mudou
    else if(mudaS == 1){
